...
class gui_pathConverter(QtWidgets.QMainWindow):
    ...
    def __changeSelection_writes(self):
	    ...
	    #### Disable Column Test ###
	    ############################
	    curWrite_node = self.WRITES[writes_rowIndex]
	    writes_disableCell = self.table_writeNodes.item(writes_rowIndex, 2)
	    print writes_disableCell #problem is that this is always a 'none' object type. why?
	    curWrite_node['disable'].setValue(writes_disableCell.isSelected())
	    #if curWrite_node['disable'].value() == 1:
		#writes_disableCell.isSelected(0)
	    #elif curWrite_node['disable'].value() == 0:
		#writes_disableCell.isSelected(1)
	    #if writes_disableCell.isSelected():
		#curWrite_node['disable'].setValue(1)
	    #elif not writes_disableCell.isSelected:
		#curWrite_node['disable'].setValue(0)
    ...
    def coreFunction(self):
        ...
        self.table_readNodes.setHorizontalHeaderLabels(['Absolute Pathnames', 'Relative Pathnames', 'Disable/Enable'])
        ...
        self.table_writeNodes.setHorizontalHeaderLabels(['Absolute Pathnames', 'Relative Pathnames', 'Disable/Enable'])
